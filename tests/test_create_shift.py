...
class test_create_shift(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.doc1 = Doctor('Marcelo', [1, 3, 5], [5], True, 3, True)
        self.doc2 = Doctor('Jonas', [1], [1, 3, 5], True, 3, True)
    ...
    def test_create_shift(self):
        ...
        #when
        shift_df, other_data_df = self.shift.best_shift(1)
        ...
        self.assertEqual(other_data_df[0][1], 0)
        # test contagem de plantões totais (solução ótima - plantoes iguais)
        self.assertEqual(other_data_df[2][1], 3)
        self.assertEqual(other_data_df[3][1], 3)
    ...
    def test_doctor_not_enough(self):
        ...
        #when
        shift_df, other_data_df = shift1.best_shift(1)
    ...
    def test_daily_shift_only(self):
        ...
        self.assertEqual(shift_df["1° Plantonista"][1], 'Marcelo')
        self.assertEqual(other_data_df[0][1], 0.5)

    def test_pontuatio_system(self):

        #when
        shift_df, other_data_df = self.shift.best_shift(1)
        pontuation = 2*(3*(200 + 100 + 100))

        #then
        self.assertEqual(other_data_df[1][1], pontuation)